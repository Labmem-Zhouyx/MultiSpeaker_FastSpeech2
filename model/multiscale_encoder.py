...
class MultiScaleEncoder(nn.Module):
    def __init__(self, in_dim=80, gru_units=128, conv_channels=[32, 32, 64, 64, 128, 128], kernel_size=3, strides=[2, 1, 2, 1, 2, 2], padding=1, global_out_dim=256, local_out_dim=3):
        ...
        # GRU
        out_channels = self.calculate_channels(in_dim, kernel_size, strides, padding, K)
        ...
        self.global_outlayer = nn.Sequential(
            nn.Linear(in_features=gru_units,
                      out_features=global_out_dim),
            nn.Tanh(),
        )

        self.local_outlayer = nn.Sequential(
            nn.Linear(in_features=gru_units,
                      out_features=local_out_dim * 2),
            nn.Tanh()
        )
    ...
    def forward(self, inputs, input_lengths=None):
        ...
        global_out = self.global_outlayer(out.squeeze(0))
        local_out = self.local_outlayer(memory)
